...
const MangoHubAPI = {
  ...
  _apiType: 'longform', // 'longform' | 'longform-v2' | 'longform-v3' | 'shortform' | 'mangomaker'

  get apiPrefix() {
    return `/api/${this._apiType}`;
  },

  // 🎬 movie(무비스튜디오)는 별도 도메인·무인증. 그 외는 mangois.love.
  get base() {
    return this._apiType === 'movie' ? 'https://movie.mangois.love' : this.BASE_URL;
  },
  ...
  setApiType(type) {
    ...
    else if (type === 'longform-v3') this._apiType = 'longform-v3';   // 🆕 롱폼V2 (V3) — long2.html
    else if (type === 'movie') this._apiType = 'movie';   // 🎬 무비스튜디오 (movie.mangois.love, 무인증)
