...
class AutomationStateMachine {
  ...
  markSuccess(result) {
    this.results.push({ success: true, index: this.currentIndex, ...result });
    this.retryCount = 0;
  ...
  markError(error) {
    ...
    if (this.retryCount >= this.maxRetries) {
      this.results.push({ success: false, index: this.currentIndex, error: this.error });
      this.retryCount = 0;
      this.transition(AutoState.COOLDOWN);
    } else {
  ...
  skipCurrent() {
    this.results.push({ success: false, index: this.currentIndex, error: 'Skipped' });
    this.retryCount = 0;
    this.transition(AutoState.COOLDOWN);
  }
