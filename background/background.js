...
2. 장소, 배경, 분위기, 조명, 카메라 앵글은 그대로 유지하세요.
3. 오직 검열에 걸릴 수 있는 표현(폭력, 선정성, 혐오 관련)만 최소한으로 수정하세요.
4. 가능하면 문제가 되는 단어 1~2개만 부드러운 동의어로 교체하세요.
5. 프롬프트의 전체 길이와 구조를 유지하세요. 내용을 삭제하거나 요약하지 마세요.
6. 수정된 프롬프트만 출력하세요. 설명, 부연, 따옴표 없이 프롬프트 텍스트만 반환하세요.`
        },
        {
          role: 'user',
